...
public class AdvancedJokeListTest extends
	...
	public void testAddJokeViaButton() {
		...
		jokes = this.retrieveHiddenMember("jokes", jokes, getActivity());
		EditText input = null;
		Button button = null;
		final EditText inputText = this.retrieveHiddenMember("textInput",
				input, getActivity());
		final Button jokeButton = this.retrieveHiddenMember("addButton",
				button, getActivity());
		assertEquals("Should be 3 default jokes", 3, jokes.size());
		getActivity().runOnUiThread(new Runnable() {
			public void run() {
				inputText.setText("This is a test joke");
				jokeButton.performClick();
			}
		});
		// wait for the request to go through
		getInstrumentation().waitForIdleSync();
		assertEquals("Should be 4 jokes now", 4, jokes.size());
		assertEquals("Ensure the joke we added is really there",
				"This is a test joke", jokes.get(3).getText());
		...
		assertEquals("Text view should also have the new joke",
				"This is a test joke", tv.getText());
	}

	@SmallTest
	public void testAddJokeViaReturn() {
		ArrayList<Joke> m_arrJokeList = null;
		m_arrJokeList = this.retrieveHiddenMember("jokes", m_arrJokeList,
				getActivity());
		EditText et = null;
		...
		tv = this.retrieveHiddenMember("m_vwJokeText", tv, jv);
		assertEquals("Text view should also have the new joke",
				"This is a second test joke", tv.getText());
	}
	...
	public void testAddJokeViaDPadTrackBall() {
		...
		tv = this.retrieveHiddenMember("m_vwJokeText", tv, jv);
		assertEquals("Text view should also have the new joke",
				"This is a third test joke", tv.getText());
	}
	...
	/*
	 * @SmallTest public void testTextViewSize() { LinearLayout jokeLayout =
	 * null; jokeLayout =
	 * this.retrieveHiddenMember("jokeLayout",jokeLayout,getActivity());
	 * JokeView jv = (JokeView)jokeLayout.getChildAt(1); TextView tv = null; tv
	 * = this.retrieveHiddenMember("m_vwJokeText",tv,jv);
